(function () {
    ...
    ready(() => {
        ...
        function confirmAction() {
            ...
            setTimeout(() => {
                document.querySelector('.ns-dialog-button-ok.button-ok')?.dispatchEvent(
        ...
        function toggleLabels() {
            const isAlwaysVisible = sidebar.classList.toggle('os-labels-always-visible');
        ...
        buttons.appendChild(makeButton('keyboard', 'Keyboard', () => setTimeout(showKeyboard, 100)));
        buttons.appendChild(makeButton('fullscreen', 'Fullscreen', toggleFullscreen));
        buttons.appendChild(makeButton('home', 'Home', () => {
            window.location.href = 'https://cad.onshape.com/documents';
        }));
        buttons.appendChild(makeButton('space', 'Clear', () => pressKey(' ', { code: 'Space', keyCode: 32, which: 32 })));
        buttons.appendChild(makeButton('esc', 'Cancel', () => pressKey('Escape', { code: 'Escape', keyCode: 27, which: 27 })));
        buttons.appendChild(makeButton('check', 'Confirm', () => setTimeout(confirmAction, 100)));
        buttons.appendChild(makeButton('search', 'Shortcut Menu', () => pressKey('s', { code: 'KeyS', keyCode: 83, which: 83 })));
        buttons.appendChild(makeButton('normal', 'Normal To', () => pressKey('n', { code: 'KeyN', keyCode: 78, which: 78 })));
        buttons.appendChild(makeButton('undo', 'Undo', () => pressKey('z', { code: 'KeyZ', keyCode: 90, which: 90, ctrlKey: true })));
        buttons.appendChild(makeButton('redo', 'Redo', () => pressKey('y', { code: 'KeyY', keyCode: 89, which: 89, ctrlKey: true })));
        buttons.appendChild(makeButton('delete', 'Delete', () => pressKey('Delete', { code: 'Delete', keyCode: 46, which: 46 })));
